...
export function clipDetail(text, max = DETAIL_MAX) {
...
export function lastThoughtLine(message) {
...
export function formatWorkingMessage(snap) {
    ...
    if (snap.phase === "writing")
        return `Writing · ${elapsed}`;
    if (snap.thought)
        return `Thinking · ${snap.thought} · ${elapsed}`;
    return `Thinking · ${elapsed}`;
}
export function installWorkingStatus(pi) {
    let ctx;
    let startedAt = 0;
    let phase = "idle";
    let thought;
    ...
    const paint = (now = Date.now()) => {
        if (!ctx || ctx.mode !== "tui" || phase === "idle")
            return;
